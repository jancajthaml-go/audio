...
public class BufferTools
{
	...
	{
		try { return byteBufferToString(bytes, offset, length, defaultCharsetName); }
	...
	{ return (length < 1)?"":new String(bytes, offset, length, charsetName); }
	
	public static byte[] stringToByteBufferIgnoringEncodingIssues(String s, int offset, int length)
	{
		try { return stringToByteBuffer(s, offset, length); }
		catch (UnsupportedEncodingException e) { return null; }
	}
	...
	{ return s.substring(offset, offset + length).getBytes(charsetName); }
	...
	public static String trimStringRight(String s)
	{
		...
		else								return s.substring(0, endPosition + 1);
	}
	...
	{ return (byte)(value?((b | ((byte)0x01 << bitPosition))):((b & (~ ((byte)0x01 << bitPosition))))); }
	
	public static int shiftByte(byte c, int places)
	{
		if (places < 0)			return (c & 0xff) << -places;
		else if (places > 0)	return (c & 0xff) >> places;
		else					return (c & 0xff);
	}
