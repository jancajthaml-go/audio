...
public abstract class AbstractID3v2Tag implements ID3v2
{
	...
	private void unpackTag(byte[] bytes) throws NoSuchTagException, UnsupportedTagException, InvalidDataException
	{
		...
		catch (ArrayIndexOutOfBoundsException e) { throw new InvalidDataException("Premature end of tag", e); }
	}
	...
	protected int unpackFrames(byte[] bytes, int offset, int framesLength)
	{
		...
		while (currentOffset <= framesLength)
		{
			try
			{
				ID3v2Frame frame = createFrame(bytes, currentOffset);
	...
	{ return obseleteFormat?new ID3v2ObseleteFrame(bytes, currentOffset):new ID3v2Frame(bytes, currentOffset); }
	
	protected ID3v2Frame createFrame(String id, byte[] data)
	{ return obseleteFormat?new ID3v2ObseleteFrame(id, data):new ID3v2Frame(id, data); }
	...
	{
		try { BufferTools.stringIntoByteBuffer(TAG, 0, TAG.length(), bytes, offset); }
		...
		if (s.length > 0)
		{
			...
			bytes[offset + MAJOR_VERSION_OFFSET] = Byte.parseByte(s[0]);
		}
		
		if (s.length > 1)
		{
			
			bytes[offset + MINOR_VERSION_OFFSET] = Byte.parseByte(s[1]);
		}
	...
	public int getGenre() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? "TCO" : "TCON");
	...
	public void setGenre(int genre) {
		if (genre >= 0) {
			...
			addFrame(createFrame("TCON", frameData.toBytes()), true);
		} else {
			// TODO remove frame?
		}
	}
	...
	public String getComment() {
		ID3v2CommentFrameData frameData = extractCommentFrameData(obseleteFormat ? "COM" : "COMM", false);
	...
	public void setComment(String comment) {
		if (comment != null && comment.length() > 0) {
			...
			addFrame(createFrame("COMM", frameData.toBytes()), true);
		}
	}
	
	public String getItunesComment() {
		ID3v2CommentFrameData frameData = extractCommentFrameData(obseleteFormat ? "COM" : "COMM", true);
		if (frameData != null && frameData.getComment() != null) return frameData.getComment().toString();
		return null;
	}
	...
	public void setItunesComment(String itunesComment) {
		if (itunesComment != null && itunesComment.length() > 0) {
			invalidateDataLength();
			ID3v2CommentFrameData frameData = new ID3v2CommentFrameData(useFrameUnsynchronisation(), "iTunNORM", null, new EncodedText(itunesComment));
			addFrame(createFrame("COMM", frameData.toBytes()), true);
		}
	}

	public String getComposer() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? "TCM" : "TCOM");
		if (frameData != null && frameData.getText() != null) return frameData.getText().toString();
		return null;
	}
	...
	public void setComposer(String composer) {
		if (composer != null && composer.length() > 0) {
			...
			addFrame(createFrame("TCOM", frameData.toBytes()), true);
		}
	}
	
	public String getPublisher() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? "TBP" : "TPUB");
		if (frameData != null && frameData.getText() != null) return frameData.getText().toString();
		return null;
	}
	...
	public void setPublisher(String publisher) {
		if (publisher != null && publisher.length() > 0) {
			...
			addFrame(createFrame("TPUB", frameData.toBytes()), true);
		}
	}
	
	public String getOriginalArtist() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? "TOA" : "TOPE");
		if (frameData != null && frameData.getText() != null) return frameData.getText().toString();
		return null;
	}
	...
	public void setOriginalArtist(String originalArtist) {
		if (originalArtist != null && originalArtist.length() > 0) {
			...
			addFrame(createFrame("TOPE", frameData.toBytes()), true);
		}
	}

	public String getCopyright() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? "TCR" : "TCOP");
		if (frameData != null && frameData.getText() != null) return frameData.getText().toString();
		return null;
	}
	...
	public void setCopyright(String copyright) {
		if (copyright != null && copyright.length() > 0) {
			...
			addFrame(createFrame("TCOP", frameData.toBytes()), true);
		}
	}

	public String getUrl() {
		ID3v2UrlFrameData frameData = extractUrlFrameData(obseleteFormat ? "WXX" : "WXXX");
	...
	public void setUrl(String url) {
		if (url != null && url.length() > 0) {
			...
			addFrame(createFrame("WXXX", frameData.toBytes()), true);
		}
	}
	...
	public String getEncoder() {
		ID3v2TextFrameData frameData = extractTextFrameData(obseleteFormat ? ID_ENCODER_OBSELETE: "TENC");
		if (frameData != null && frameData.getText() != null) return frameData.getText().toString();
		return null;
	}
	...
	public void setEncoder(String encoder) {
		if (encoder != null && encoder.length() > 0) {
			...
			addFrame(createFrame("TENC", frameData.toBytes()), true);
		}
	}
	
	public byte[] getAlbumImage() {
		ID3v2PictureFrameData frameData = createPictureFrameData(obseleteFormat ? "PIC" : "APIC");
	...
	public void setAlbumImage(byte[] albumImage, String mimeType) {
		if (albumImage != null && albumImage.length > 0 && mimeType != null && mimeType.length() > 0) { 
			...
			addFrame(createFrame("APIC", frameData.toBytes()), true);
		}
	}
		...
		if (frameSet != null) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					if (itunes && "iTunNORM".equals(frameData.getDescription().toString())) {
						return frameData;
					} else if (! itunes) {
						return frameData;
					}
